...
async function translateText(text, targetLang, sourceLang = 'pt') {
    ...
    if (!hasQuotaAvailable()) {
        console.warn('Quota diária excedida. Retornando texto original.');
        return text;
    }
...
async function translateObject(obj, targetLang, sourceLang = 'pt', depth = 0, parentKey = '') {
    ...
    if (depth > 10) {
        console.warn('Profundidade máxima atingida na tradução');
        return obj;
    }
    ...
    if (typeof obj === 'string') {
        ...
        console.log(`Traduzindo: "${obj.substring(0, 50)}${obj.length > 50 ? '...' : ''}"`);
...
async function changeLanguage(language) {
    ...
    try {
        ...
        if (language === 'pt-br') {
            ...
            console.log('Traduzindo dados com Google Translate...');
